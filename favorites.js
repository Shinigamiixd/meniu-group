...
// const the_favorites = localStorage.getItem('favorite')
...
const the_favorites = (() => {
  ...
  console.log(the_favorites);

const patiekalai = ['All'];

for (let j = 0; the_favorites.length > j; j++) {
    console.log(the_favorites[j].id);
    for (let i = 0; newarr.length > i; i++) {
        const newarr = menu[i].id
        if (newarr.includes(the_favorites[j].id)) {
        console.log(menu);
        }  
    }
